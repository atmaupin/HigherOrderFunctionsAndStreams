...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testFilter() {
        ...
        List<String> actual = input.filter(s -> s.charAt(0) == 'b').collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(str -> str.charAt(str.length() - 1) % 2 == 0).reduce("", (string, result) -> string + result);
    ...
    public void testCountByDate() {
        ...
        long actual = input.filter(r -> instant.isAfter(r.timestamp)).count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesByMonth() {
        Stream<Revision> input = getRevisions("soup04.json");
        Map<Month, Long> actual = input.collect(Collectors.groupingBy(this::getMonth, Collectors.counting()));
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = input.collect(Collectors.groupingBy(this::getMonth, Collectors.counting())).entrySet().stream().max(Map.Entry.comparingByValue()).get().getKey();
